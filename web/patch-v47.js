...
// patch-v47: tooltips + radar (CSP-safe, no inline)
(function(){
  ...
  function ensureTooltips(){
    try{
      const map = {
        ...
        model: 'System/model label.',
        accuracy: 'Primary accuracy metric.',
        grounding: 'Evidence linkage quality.',
        safety: 'Safety policy conformance.',
        robustness: 'Robustness to perturbations.',
        bias: 'Distributional fairness proxy.',
        calibration: 'Confidence calibration.',
        trust: 'Composite trust score.',
        audit: 'Quick audit trail preview.',
        provenance: 'Provenance proof bundle.',
  ...
  function drawRadarTop(){
    try{
      ...
      function valFor(name){
        ...
        if(!m) return 0.6;
        var v=parseFloat(m[1]); if(isNaN(v)) return 0.6;
