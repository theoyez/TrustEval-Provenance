...
async function boot(){
  ...
  function apply(){
    ...
    let view = rows;
    if(q){ view = rows.filter(r=> r.model.toLowerCase().includes(q)); }
    renderBoard(view);
  }
  ...
  if(runA && runB){
    rows.forEach(r=>{
      const o1 = document.createElement('option'); o1.value=r.id; o1.textContent=r.id;
      const o2 = document.createElement('option'); o2.value=r.id; o2.textContent=r.id;
