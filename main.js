...
function auditCell(r){
  ...
  const src = (aq.sources||[]).slice(0,3).join(', ');
  ...
  return `<div class="audit">
    <span class="badge chev" title="retrieved@k">k=${aq.retrieved_k}</span>
    <span class="badge chev" title="top sources">${src}</span>
    <span class="badge" title="consistency">${badge}</span>
  </div>`;
}
function renderBoard(rows){
  ...
  rows.forEach(r=>{
    ...
    tr.innerHTML = `
      ...
      ${window.TRUSTEVAL_HAS_PROV ? `<td>${auditCell(r)}</td>` : ''}
...
async function boot(){
  ...
  if(runA && runB){
    ...
    document.getElementById('swap').addEventListener('click', ()=>{
      const t=runA.value; runA.value=runB.value; runB.value=t; renderCards();
    });
    function renderCards(){
      ...
      d.innerHTML=`
        <div class="kpi"><span class="label">Δ Accuracy</span><span class="value">${fmt(a.metrics.accuracy-b.metrics.accuracy)}</span></div>
        <div class="kpi"><span class="label">Δ Perplexity</span><span class="value">${fmt(a.metrics.perplexity-b.metrics.perplexity)}</span></div>`;
      cards.appendChild(d);
    }
